...
class OpticalDesignApp:
    def __init__(self, root):
        ...
        tk.Button(btn_frame, text="▶ RUN FDTD SIMULATION", command=self.run_fdtd, 
    ...
    def on_comp_change(self, event):
        ...
        if comp == "Straight Guide":
            self.add_entry("len_cm", "Length (cm):", "1.0")
    ...
    def run_fdtd(self):
        params = self.get_params()
        try:
            messagebox.showinfo("FDTD", "Launching FDTD Simulation...\n(Geometry scaled for visualization)")
            fdtd_sim.run_fdtd_demo(params)
        except Exception as e:
            messagebox.showerror("FDTD Error", f"Missing libraries\n{e}")
